...
int PM_AnimLength( int index, animNumber_t anim )
{
	...
	return level.knownAnimFileSets[index].animations[anim].numFrames * fabsf(level.knownAnimFileSets[index].animations[anim].frameLerp);
}
...
*/
		...
		{//FIXME: allow to set a specific time?
			...
			{	// Make sure to only wait in full 1/20 sec server frame intervals.
				...
				dur = (animations[anim].numFrames -1) * fabsf(animations[anim].frameLerp);
				//dur = ((int)(dur/50.0)) * 50;
				//dur -= blendTime;
				if (dur > 1)
				{
				...
				{
					PM_SetTorsoAnimTimer( gent, torsoAnimTimer, fabsf(animations[anim].frameLerp) );
				}
			}
			else
			{
				PM_SetTorsoAnimTimer( gent, torsoAnimTimer, (animations[anim].numFrames ) * fabsf(animations[anim].frameLerp) );
			}
		}
	}
...
*/
		...
		{//FIXME: allow to set a specific time?
			...
			{	// Make sure to only wait in full 1/20 sec server frame intervals.
				...
				{
					PM_SetLegsAnimTimer( gent, legsAnimTimer, fabsf(animations[anim].frameLerp) );
				}
			}
			else
			{
				PM_SetLegsAnimTimer( gent, legsAnimTimer, (animations[anim].numFrames ) * fabsf(animations[anim].frameLerp) );
			}
		}
	}
